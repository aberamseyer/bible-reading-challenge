...
function createUser(id, position, name, emoji, zIndex) {
  ...
  newUser.innerHTML = `${emoji}<small>${name
    .split(" ")
    .map((x) => x.charAt(0))
    .join("")
    .toUpperCase()}</small>`;
        ...
        switch (data.type) {
          case "init-client":
            ...
            for (let person of data.people) {
              people.set(
                person.id,
                createUser(person.id, person.position, person.name, person.emoji, zIndex++),
              );
            }
          ...
          case "add-user":
            ...
            people.set(
              data.id,
              createUser(data.id, data.position, data.name, data.emoji, people.size + 2),
            );
            break;
          case "move-user":
