...
CREATE VIEW read_dates_dated AS
SELECT
  rd.id,
  rd.user_id,
  u.name,
  rd.timestamp,
  DATETIME (rd.timestamp, 'unixepoch') date,
  sd.passage
FROM
  read_dates rd
...
SELECT
  ...
  sd.date,
  sd.passage,
  b.name || ' ' || c.number || ':' || v.number reference,
  b.id book_id,
  b.name book_name,
  c.id chapter_id,
  ...
  v.id verse_id,
  JSON_EXTRACT (je.value, '$.s') verse_start,
  JSON_EXTRACT (je.value, '$.e') verse_end,
  rcv,
  kjv,
  niv,
  esv,
...
WHERE
  v.number BETWEEN JSON_EXTRACT (je.value, '$.s') AND JSON_EXTRACT  (je.value, '$.e')
